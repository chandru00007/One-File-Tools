...
Run the build script to regenerate the landing page from the template, then open `index.html` to verify your tool appears:
...
> **Note:** `build.js` reads `index-template.txt` and injects tool data from `tools.json` to generate `index.html`. You do **not** need to edit the template, the landing page, or the Available Tools table in `ReadMe.md` — those are auto-generated by the build scripts and `node sync-readme.js`, which the maintainer runs after merging your PR.
